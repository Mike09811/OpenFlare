        ...
        <div className="rounded-[26px] border border-[var(--border-default)] bg-[var(--surface-elevated)] p-5">
            <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                    <p className="text-xs font-medium uppercase tracking-[0.22em] text-[var(--foreground-muted)]">
                        {label}
                    </p>
                    <p className="text-2xl font-semibold text-[var(--foreground-primary)]">{value}</p>
                </div>
                <span
                    className="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-[var(--border-default)] bg-[var(--surface-panel)] text-[var(--foreground-primary)]">
...
function TabButton({
    ...
    return (
        ...
        >
            <p className=" text-sm font-semibold text-[var(--foreground-primary)]">{label}</p>
        </button>
    );
}
...
function RuleListSection({
    ...
    return (
        <div className="rounded-[26px] border border-[var(--border-default)] bg-[var(--surface-elevated)] p-5">
            <div className="flex items-start justify-between gap-4">
                <div>
                    <h3 className="text-sm font-semibold text-[var(--foreground-primary)]">{title}</h3>
          ...
          {items.length}
            ...
            {items.length > 0 ? (
                <div className="mt-4 flex flex-wrap gap-2">
                    {items.map((item) => (
                        <RuleChip key={item} label={item} tone={tone} onRemove={() => onRemove(item)}/>
                    ))}
                </div>
            ) : (
                <div
    ...
    return (
        ...
        >
            <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <button
                                    key={option.value}
                                    type="button"
                                    onClick={() => onChange({listType: option.value as RuleListType})}
                                    className={cn(
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <button
                                    key={option.value}
                                    type="button"
                                    onClick={() => onChange({dimension: option.value as RuleDimension})}
                                    className={cn(
                                        'rounded-2xl border px-4 py-3 text-sm font-medium transition',
                ...
                {state.dimension === 'ip' ? (
                    <ResourceField label="IP / IP 段" hint="支持单个 IP、CIDR，或使用换行/逗号一次添加多个。">
                        <ResourceTextarea
                            value={state.ipValue}
                    ...
                    <div className="space-y-4">
                        <div
                            className="flex items-center gap-3 rounded-2xl border border-[var(--border-default)] bg-[var(--surface-elevated)] px-4 py-3">
...
export function WAFPage() {
    ...
    const totalRules = groups.reduce((sum, group) => sum + countRuleEntries(group), 0);
  ...
  const appliedSiteNames = selectedGroup?.is_global
    ...
    : (selectedGroup?.applied_site_ids ?? [])
        .map((id) => routes.find((route) => route.id === id)?.site_name ?? `网站 #${id}`)
        .sort((left, right) => left.localeCompare(right));
    ...
    const applyRuleModal = () => {
        ...
        if (values.length === 0) {
            setFeedback({
                tone: 'danger',
                message: ruleModal.dimension === 'ip' ? '请先输入 IP 或 IP 段。' : '请先选择地域。',
            });
            return;
        }
        ...
        setDraft((current) =>
            updateDraftList(current, listKey, (items) => normalizeItems([...items, ...values])),
        );
        setFeedback({tone: 'info', message: '名单项已添加到当前草稿，保存后生效。'});
        closeRuleModal();
        setActiveTab('lists');
    };
    ...
    const overviewItems: Array<{ label: string; value: ReactNode }> = [
        ...
        {
            label: '生效范围',
            value: selectedGroup?.is_global ? '全部网站' : `${selectedGroup?.applied_site_count ?? 0} 个网站`,
        },
        {
            label: '拦截返回',
            ...
            <div className="space-y-6">
                ...
                <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
                  ...
                  <span className="flex items-center justify-between gap-3">
                    ...
                    {group.is_global ? '应用全部网站' : `已应用 ${group.applied_site_count} 个网站`} ·{' '}
                            ...
                            {selectedGroup?.is_global ? '全局' : '自定义'}
                                                ...
                                                <dl className="mt-5 grid gap-4 sm:grid-cols-2">
                                                    {overviewItems.map((item) => (
                                                        ...
                                                        >
                                                            <dt className="text-xs font-medium uppercase tracking-[0.18em] text-[var(--foreground-muted)]">
                                                                {item.label}
                                                            </dt>
